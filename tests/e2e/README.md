...
In this scenario, you want to run the E2E test suite against the operator image corresponding to the `VERSION` declared in the project [`Makefile`](../../Makefile), which should be publicly available at `quay.io/rhdh/rhdh-rhel10-operator:<VERSION>` on `main` (or `quay.io/rhdh/rhdh-rhel9-operator:<VERSION>` on maintenance branches).
...
```shell
# if the tag is already published and available at the default location: quay.io/rhdh/rhdh-rhel10-operator (main) or quay.io/rhdh/rhdh-rhel9-operator (maintenance branches)
