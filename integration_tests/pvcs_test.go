...
var _ = When("create backstage PVCs configured", func() {
	...
	It("creates PV dynamically with configured by default PVC", func() {
		...
		Eventually(func(g Gomega) {
			...
			//g.Expect(path).To(BeMountedToContainer(depl.Spec.Template.Spec.Containers[model.BackstageContainerIndex(&depl.Spec.Template.Spec)]))
			g.Expect(path).To(BeMountedToContainer(backstageContainer2(bpod.Spec)))
			...
			_, _, err = executeRemoteCommand(ctx, ns, bpod.Name, backstageContainer2(bpod.Spec).Name, fmt.Sprintf("test -d %s", path))
			g.Expect(err).ShouldNot(HaveOccurred())
	...
	It("bounds configured by default PVC with precreated PV", func() {
		...
		Eventually(func(g Gomega) {
			...
			g.Expect(err).ShouldNot(HaveOccurred())
			path := filepath.Join(model.DefaultMountDir, utils.ToRFC1123Label(model.PvcsName(backstageName, "myclaim1")))
			g.Expect(path).To(BeMountedToContainer(backstageContainer(*depl)))
			g.Expect(utils.ToRFC1123Label(model.PvcsName(backstageName, "myclaim1"))).
			...
			// check if mounted directory is there
			_, _, err = executeRemoteCommand(ctx, ns, pod.Name, backstageContainer(*depl).Name, fmt.Sprintf("test -d %s", path))
			g.Expect(err).ShouldNot(HaveOccurred())
	...
	It("creates specified PVCs and mounts to container", func() {
		...
		Eventually(func(g Gomega) {
			...
			path2 := "/my/pvc2/path"
			g.Expect(path2).To(BeMountedToContainer(backstageContainer(*depl)))
			...
			_, _, err = executeRemoteCommand(ctx, ns, pod.Name, backstageContainer(*depl).Name, fmt.Sprintf("test -d %s", path))
			g.Expect(err).ShouldNot(HaveOccurred())
			_, _, err = executeRemoteCommand(ctx, ns, pod.Name, backstageContainer(*depl).Name, fmt.Sprintf("test -d %s", path2))
			g.Expect(err).ShouldNot(HaveOccurred())
