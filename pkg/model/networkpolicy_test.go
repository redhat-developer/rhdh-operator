...
func TestDefaultNetworkPolicies(t *testing.T) {
	...
	assert.Equal(t, 11, len(mo.Items), "expected 8 backend + 3 DB NPs")
	...
	assert.Equal(t, 8, backendCount, "expected 8 backend NPs")
...
func TestNetworkPoliciesWithoutLocalDb(t *testing.T) {
	...
	mo := obj.Object().(*multiobject.MultiObject)
	assert.Equal(t, 8, len(mo.Items), "expected only 8 backend NPs when localDb is disabled")
...
func TestNetworkPolicyFlavourMergePreservesNamespaceWidePodSelector(t *testing.T) {
	...
	mo := obj.Object().(*multiobject.MultiObject)
	assert.Equal(t, 9, len(mo.Items), "expected 8 backend + 1 flavour NP (DB NPs filtered out)")
