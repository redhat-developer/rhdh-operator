...
and can push the operator images and plugin OCI artifacts directly to the mirror registry and the cluster.
...
The script handles both core RHDH operator images (rhdh-hub, operator, operator-bundle) and dynamic plugins delivered as OCI artifacts.
Plugins can be sourced from a plugin catalog index or from a custom plugin list file.
...
Options:
  ...
  --plugin-index <oci-url>               : Plugin catalog index OCI artifact (e.g., oci://quay.io/rhdh-plugin-catalog/backstage-community-plugin-acr:1.8)
