...
# 3. `--olm-version` defaults to `auto`: on clusters with OLM v1 CRDs (for example OpenShift 4.21+),
#    the script installs via ClusterCatalog + ClusterExtension; otherwise it uses CatalogSource + Subscription (OLM v0).
#    Force with `--olm-version v0` or `--olm-version v1` when needed.
# 4. With `--use-oc-mirror true`, catalog resources come from oc-mirror's native manifests under
#    `working-dir/cluster-resources/` (`cc-*.yaml` for OLM v1, `cs-*.yaml` for OLM v0).
...
# 5. See the GA notes above for `--olm-version` and oc-mirror catalog manifests (`cc-*.yaml` / `cs-*.yaml`).
...
# 1. Omit `--to-registry` if you are connected against an OpenShift cluster and want the script to expose the internal cluster registry and use that for mirroring.
# 2. Omit `--use-oc-mirror` if you don't want to use `oc-mirror`. `oc-mirror` makes sense only if targetting OpenShift.
# 3. `--olm-version` defaults to `auto` (ClusterCatalog + ClusterExtension on OLM v1 clusters, CatalogSource + Subscription otherwise).
#    With `--use-oc-mirror true`, the catalog comes from native `cc-*.yaml` (v1) or `cs-*.yaml` (v0) under `working-dir/cluster-resources/`.
