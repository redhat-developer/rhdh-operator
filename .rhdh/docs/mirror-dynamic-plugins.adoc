...
Create a ConfigMap containing the mirroring configuration. See link:https://github.com/containers/image/blob/main/docs/containers-registries.conf.5.md[Configuring registries] for more information about the format.
...
Depending on your target mirror registry, you may need to configure a policy file. For production environments, you should use a more restrictive policy that verifies image signatures. See link:https://docs.redhat.com/en/documentation/red_hat_enterprise_linux/9/html/building_running_and_managing_containers/assembly_signing-container-images_building-running-and-managing-containers[Signing container images] for more details.
...
* *Custom certificates*: See link:https://docs.redhat.com/en/documentation/red_hat_developer_hub/1.8/html/installing_and_viewing_plugins_in_red_hat_developer_hub/rinstall-plugins-from-oci-registries-by-using-custom-certificates[Install plugins from OCI registries by using custom certificates] if the target mirror registry uses custom certificates that RHDH should be configured to trust. This requires mounting additional extra files in the RHDH init container.
