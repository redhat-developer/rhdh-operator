...
For Operator deployments, configure the `Backstage` Custom Resource with the mirrored plugin references. For Helm deployments, update your `values.yaml` file with the mirrored plugin references. See link:https://access.redhat.com/documentation/en-us/red_hat_developer_hub[Red Hat Developer Hub Documentation] for configuration details.
